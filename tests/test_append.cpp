...
TEST(append_immmutable_array_nonempty) {
    int init[] = {1, 2, 3};
...
TEST(append_mutable_list_nonempty){
    int init[] = {1, 2, 3};
...
TEST(append_immmutable_list_nonempty){
    int init[] = {1, 2, 3};
...
TEST(append_mutable_array_empty){
...
TEST(append_immmutable_array_empty){
...
TEST(append_mutable_list_empty) {
...
TEST(append_mutable_array_one_element){
    int init[] = {32};
    array_seq<Mutability::Mutable, int>* seq = new array_seq<Mutability::Mutable, int>(init, 1);
    assert(seq->size() == 1);
    seq->append(26);
    assert(seq->size() == 2);
    assert(seq->get_first() == 32);
    assert(seq->get_last() == 26);
    delete seq;
}

TEST(append_immmutable_array_one_element){
    int init[] = {32};
    ...
    assert(new_seq->get_first() ==32);
    assert(new_seq->get_last() == 26);
    delete seq;
    delete new_seq;
}

TEST(append_mutable_list_one_element) {
    int init[] = {32};
    ...
    assert(seq->size() == 2);
    assert(seq->get_first() == 32);
    assert(seq->get_last() == 42);
    delete seq;
}

TEST(append_immutable_list_one_element) {
    int init[] = {32};
